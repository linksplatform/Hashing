...
// https://github.com/komrad36/CRC/blob/master/LICENSE (MIT License) by Kareem Omar
...
uint32_t crc32(const void* M, uint32_t bytes, uint32_t prev) {
    ...
    while (bytes >= LEAF_SIZE_INTEL) {
        ...
        const __m128i vK = _mm_cvtepu32_epi64(_mm_loadu_si128((__m128i*)(&g_lut[n - 1])));
...
static constexpr uint32_t g_lut[] = {
...
#define CRC_ITERS_128_TO_2()                                \
    do {                                                    \
        X0(128) X1(126) X2(122) X3(114) X4(98) X5(66) X6(2) \
    } while (0)
...
uint32_t crc32(const void* M, uint32_t bytes, uint32_t prev /* = 0*/) {
    ...
    while (bytes >= LEAF_SIZE_AMD) {
        ...
        crcA = _mm_crc32_u64(crcA, *(uint64_t*)(pA - 8));
        const __m128i vK = _mm_cvtsi32_si128(g_lut[n - 1]);
        const __m128i vA = _mm_clmulepi64_si128(_mm_cvtsi64_si128(crcA), vK, 0);
